...
static _u32 get_line(_u8 *buffer, _u32 sz) {
	_u32 r = 0;
...
static _u8 parse_line(_u8 *buffer, _u32 offset, _u32 sz, _str_t mask, _cfg_t *p_cfg) {
	_u8 r = PARSER_EMPTY_LINE;
	_u8 *bp = buffer + offset;
	_u32 bi = 0;	// index in source buffer
	_u16 mi = 0;	// index in mask
	_u16 ml = str_len(mask); // mask size
	_u8  li = 0;	// item index
	_u16 ci = 0;	// item size
...
static _u8 show_menu(_cfg_t *p_cfg, _u8 nitems) {
	_u8 r = 0xff; // invalid selection
	_u8 i = 0;
	_u8 row;
	_u32 countdown = 0;
	_u32 _countdown = 0;
	...
	_u8 spc=' ';
	_u16 key = 0xffff;
	_u8 color = MENU_COLOR;
	_u8 boot[MAX_CFG_LINES];
	...
	for(i = 0; i < nitems; i++) {
		...
		if(txt_cmp(ITEM_NAME(p_cfg[i]),STR_DISPLAY,ITEM_NAME_SZ(p_cfg[i])) == 0) {
			_g_p_stub->_display.display_text(ITEM_VALUE(p_cfg[i]),
	...
	// hide cursor
	_g_p_stub->_display.hide_cursor();
	...
	do {		
		...
		if(r != 0xff && key != 0) {
			switch(key & 0xff00) {
				...
				case KEY_UP_ARROU: // up arrow
					...
					while(row) {
						...
						for(i = 0; i < nboot; i++) {
							if(row == str2i(ITEM_ROW(p_cfg[boot[i]]),(_s8)ITEM_ROW_SZ(p_cfg[boot[i]]))) {
								r = boot[i];
								row = 0;
								break;
							}
			...
			// status line
			for(i = 2; i < 78; i++)
				_g_p_stub->_display.display_text((_str_t)&spc,1,24,i,MENU_COLOR | 0x80);
			
			for(i = 0; i < nboot; i++) {
				if(boot[i] == r)
					color = MENU_COLOR | 0x80;
				else
					color = MENU_COLOR;
					
				_g_p_stub->_display.display_text(ITEM_VALUE(p_cfg[boot[i]]),
			...
			_g_p_stub->_display.display_text(ITEM_EXEC(p_cfg[r]),ITEM_EXEC_SZ(p_cfg[r]),24,3,MENU_COLOR | 0x80);
			_g_p_stub->_display.display_text(ITEM_PARAMS(p_cfg[r]),
							(ITEM_PARAMS_SZ(p_cfg[r]) < (78 - (ITEM_EXEC_SZ(p_cfg[r]) + 4))) ? 
							(ITEM_PARAMS_SZ(p_cfg[r])) : (78 - (ITEM_EXEC_SZ(p_cfg[r]) + 4)),
							24,(_u8)(ITEM_EXEC_SZ(p_cfg[r]) + 4),MENU_COLOR | 0x80);
			
		}
		
		key = _g_p_stub->_kbd.get_key();
		_g_p_stub->_display.cursor_pos(24,0);
		_g_p_stub->_display.byte_out((_u8)(_countdown/60000));
	} while(_countdown);
...
void menu(_u8 *p_cfg, _u32 cfg_sz, _str_t p_file_name, _u16 *name_sz,_str_t p_args,_u16 *args_sz) {
	...
	if(_r != 0xff) {
		mem_cpy((_u8 *)p_file_name,(_u8 *)ITEM_EXEC(_cfg[_r]),
				(*name_sz < ITEM_EXEC_SZ(_cfg[_r])) ? *name_sz :  ITEM_EXEC_SZ(_cfg[_r]));

		*name_sz = ITEM_EXEC_SZ(_cfg[_r]);

		mem_cpy((_u8 *)p_args,(_u8 *)ITEM_PARAMS(_cfg[_r]),
				(*args_sz < ITEM_PARAMS_SZ(_cfg[_r])) ? *args_sz :  ITEM_PARAMS_SZ(_cfg[_r]));
		*args_sz = ITEM_PARAMS_SZ(_cfg[_r]);
	}
}
