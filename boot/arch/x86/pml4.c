...
void _pm_build_pml4_2m(_pml4_info_t *p) {
	...
	_u64 ram_sz = 0;
	_u8 ram_idx = 0;
	_u8 ram_chunks = (_u8)p->p_sc->mm_cnt;
	_u64 ram_pages_2m = 0;
	_u64 ram_pages_4k = 0;
	_u64 ram_byte = 0;
	...
	_u32 i,icpu, ipml4, ipdp, ipd, ipt;
	...
	/*	if(p_mmap->type == MEM_TYPE_FREE || 
				p_mmap->type == MEM_TYPE_RESERVED ||
				p_mmap->type == MEM_TYPE_UNKNOWN) */
	...
	ram_pages_4k = (ram_sz - (ram_pages_2m * PAGE2M)) / PAGE4K;
	...
	for(ipd = 0; ipd < ram_pages_2m; ipd++) {
		...
		p_pd[ipd].f.pcd = 1; // 'non cacheable'
		p_pd[ipd].f.a = 0;
		p_pd[ipd].f.d = 0;
	...
	for(ipt = 0; ipt < pt_records; ipt++) {
		...
		p_pt[ipt].f.us = 0;
		p_pt[ipt].f.pwt = 0;
		p_pt[ipt].f.pcd = 1; // 'non cacheable'
		p_pt[ipt].f.a = 0;
		p_pt[ipt].f.d = 0;
		p_pt[ipt].f.pat = 0;
		p_pt[ipt].f.g = 0;
	...
	p->p_sc->rmap[p->p_sc->rm_cnt].address = (_u64)(_u32)p->pt_addr;
	p->p_sc->rmap[p->p_sc->rm_cnt].size = table_sz;
...
void _build_pml4(_u8 *pt_addr, _startup_context_t *p_sc) {
	_pml4_info_t pi;
