...
_u32 find_string(_str_t str1,_str_t str2) {
	...
	while(l1--) {
		if(mem_cmp((_u8 *)str2,(_u8 *)(str1+i),l) == 0) 
			return i;
		i++;
	}
...
void trim_left(_str_t str) {
	...
	//memmove((void *)str,(str+i),(strlen(str+i)+1));
	mem_cpy((_u8 *)str,(_u8 *)(str+i),(str_len(str+i)+1));
}
...
static _u32 check_str1(_str_t str1,_str_t str2) {
	...
	l1 = (_u32)str_len(str1);
	l2 = (_u32)str_len(str2);
...
_u8  div_str(_str_t str,_str_t p1,_u32 sz_p1,_str_t p2,_u32 sz_p2,_cstr_t div) {
	...
	if(i != INVALID_STRING_POSITION) {
		//memmove(p1,str,(i<sz_p1)?i:sz_p1);
		mem_cpy((_u8 *)p1,(_u8 *)str,((_u32)i<sz_p1)?(_u32)i:sz_p1);
		p1[((_u32)i<sz_p1)?(_u32)i:sz_p1] = 0;
		str_cpy(p2,(str+i+1),sz_p2);

		return 1;
	}

	str_cpy(p1,str,sz_p1);
	p2[0] = 0;
	...
	for(i=0;i<l1;i++) {
		if(str[i] == ESCAPE) {
			...
			if(!ex_count) {
				for(j=0;j<l2;j++) {
					if(str[i] == div[j]) {
						mem_cpy((_u8 *)p1, (_u8 *)str, (i < (_u32)sz_p1) ? i : sz_p1);
						p1[(i<sz_p1)?i:sz_p1] = 0;
						str_cpy(p2,(str+i+1),((_s32)sz_p2 < (_s32)str_len(str+i)) ? sz_p2 : str_len(str+i));
						return 1;
					}
				}
			}
		...
		if(t) {
			switch(fmt[i]) {
				...
				case 's':  // small letters 
					...
					mem_cpy((_u8 *)buf+r, (_u8 *)_arg_str, _r);
					r += _r;
					buf[r] = 0;
					t = 0;
					break;
