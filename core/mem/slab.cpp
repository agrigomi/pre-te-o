...
static _slab_t *slab_entry(_slab_context_t *p_scxt, _u32 sz) {
	...
	for(_u32 p = 0; p < pc; p++) {
		if(_sz >= b && _sz < (b<<1)) {
			r = p_scxt->p_slab + p;
			if(r->osz == 0) {
				mem_set((_u8 *)r, 0, sizeof(_slab_t));
				r->osz = _sz;
				r->count = 0;
			}
			break;
...
bool slab_init(_slab_context_t *p_scxt) {
	bool r = false;
	if(p_scxt->p_mem_alloc && p_scxt->page_size >= SLAB_MIN_PAGE) {
		if((p_scxt->p_slab = (_slab_t *)p_scxt->p_mem_alloc(1, DEFAULT_SLAB_LIMIT, p_scxt->p_udata))) {
			mem_set((_u8 *)(p_scxt->p_slab), 0, p_scxt->page_size);
			r = true;
		}
	}
	return r;
...
static bool slab_inc_rec_level(_slab_context_t *p_scxt, _slab_t *p_slab) {
	bool r = false;
	// increase of recursion level
	_u8 *pnew = (_u8 *)p_scxt->p_mem_alloc(1, DEFAULT_SLAB_LIMIT, p_scxt->p_udata);
	if(pnew) {
		mem_set(pnew, 0, p_scxt->page_size);
		mem_cpy(pnew, (_u8 *)p_slab, sizeof(_slab_t));
...
static bool _slab_verify(_slab_t *p_slab, void *ptr, _u32 page_size, _slab_t **pp_slab, _u64 **p_obj, _u32 *p_idx) {
	bool r = false;
	for(_u8 i = 0; i < SLAB_NODES; i++) {
		if(p_slab->ptr[i]) {
			if(p_slab->level) {
...
void slab_free(_slab_context_t *p_scxt, void *ptr) {
	if(p_scxt->p_slab) {
		_slab_hlock_t hlock = p_scxt->p_lock(0, p_scxt->p_udata);
		_u32 count = p_scxt->page_size / sizeof(_slab_t);
		...
		for(_u32 i = 0; i < count; i++) {
			...
			_slab_t *_ps=0;
			_u32 idx = 0;
			if(_slab_verify(ps, ptr, p_scxt->page_size, &_ps, &p_obj, &idx)) {
				if(*p == *p_obj) {
					*p_obj = SLAB_PATTERN2;
#ifdef RELEASE_BIG_CHUNKS
					if(_ps->osz >= p_scxt->page_size) {
...
bool slab_verify(_slab_context_t *p_scxt, void *ptr, _u32 size) {
	bool r = false;
	_slab_t *p_slab = slab_entry(p_scxt, size);
...
bool slab_verify(_slab_context_t *p_scxt, void *ptr) {
	bool r = false;
	if(p_scxt->p_slab) {
		_slab_hlock_t hlock = p_scxt->p_lock(0, p_scxt->p_udata);
		_u32 count = p_scxt->page_size / sizeof(_slab_t);
		...
		_u32 idx = 0;
		for(_u32 i = 0; i < count; i++) {
			if((r = _slab_verify(ps, ptr, p_scxt->page_size, &_ps, &p_obj, &idx)))
				break;
			ps++;
		}
		p_scxt->p_unlock(hlock, p_scxt->p_udata);
...
void _destroy_slab(_slab_context_t *p_scxt, _slab_t *p_slab) {
	for(_u8 i = 0; i < SLAB_NODES; i++) {
		if(p_slab->ptr[i]) {
			if(p_slab->level) {
				_u32 count = p_scxt->page_size / sizeof(_slab_t);
	...
	mem_set((_u8 *)p_slab, 0, sizeof(_slab_t));
}
